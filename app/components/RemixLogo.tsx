...
//type Maybe<T> = T extends infer T ? Partial<T> : undefined;
//
...
// const SvgLogoWrapper = (props?: RemixLogoProps, config?: SvgLogoConfig) => {
    // let [DefaultSvgLogoProps, setDefaultSvgLogoProps] = useState<Maybe<RemixLogoProps[]>>([props] ?? undefined);
    // useEffect(() => {
    //     setDefaultSvgLogoProps([{
    //         "viewBox": props?.viewBox ?? "0 0 659 165",
    ...
    //         "aria-labelledby": props?.["aria-labelledby"] ?? "remix-run-logo-title",
    //         "role": props?.role ?? "img",
    ...
    //     }])
    // }, []);
    // Array<RemixLogoProps | undefined>.shift(): RemixLogoProps | undefined

    // let [SvgLogoProps, setSvgLogoProps] = useState<Maybe<RemixLogoProps[]>>([props] ?? undefined);
    
    // let [CustomSvgLogoProps, setCustomSvgLogoProps] = useState<Maybe<RemixLogoProps[]>>([props] ?? undefined);
    // let CustomProps: RemixLogoProps[] = [];
    // let CustomProps: Record<keyof RemixLogoProps, string>[] = [];
    // if (!props)
    // if (config && config.CustomProps) {
    //     let customProps = config.CustomProps;
    //     customProps.forEach((value, index, CustomProps) => {
    //         if (value && CustomProps[index] !== SvgLogoProps[index]) {
    //             CustomProps[index] = SvgLogoProps[index];
    //             // setSvgLogoProps(CustomProps[index])
    //         };
    //         CustomProps[index]
    //         // CustomProps.push(customProp)
    //     });
    //     // setSvgLogoProps(CustomProps);
    // }
    //return (
    //  <svg
    ...
    //    {...props}
    //  >
    //    <title id="remix-run-logo-title">{props?.title ?? "Remix Logo"}</title>
...
const RemixLogo = (props: Pick<RemixLogoProps, keyof RemixLogoProps>) => {
    return (
      <svg
        viewBox={props.viewBox ?? "0 0 659 165"}
        version={props.version ?? "1.1"}
